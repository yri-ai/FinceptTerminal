...
constexpr int kMinimumIterations = 100000;
...
QString PhaseOnePasswordHasher::hash_password(const QString& password) {
    ...
    const QByteArray hash = derive_key(password, salt, kIterations);
...
bool PhaseOnePasswordHasher::verify_password(const QString& password, const QString& encoded_hash) {
    ...
    if (!iterations_ok || iterations < kMinimumIterations)
        return false;
    ...
    return constant_time_equals(derive_key(password, salt, iterations), expected_hash);
}
